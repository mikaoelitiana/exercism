...
def send_to_store(cart: dict, aisle_mapping: dict):
    ...
    for key in cart.keys():
        fulfillment[key] = [cart[key], aisle_mapping[key][0], aisle_mapping[key][1]]
...
def update_store_inventory(fulfillment_cart, store_inventory):
    ...
    for key in fulfillment_cart.keys():
        new_value = store_inventory[key][0] - fulfillment_cart[key][0]
        store_inventory[key] = [
            new_value if new_value > 0 else "Out of Stock",
            store_inventory[key][1],
            store_inventory[key][2],
        ]
